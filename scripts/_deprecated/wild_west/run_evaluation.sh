...
DEFAULT_CONFIG="configs/evaluation/evaluation_config.yaml"
...
Examples:
  # Evaluate exp0_baseline experiment (default: serial, no timeout)
  $0 exp0_baseline
  ...
  # Use parallel evaluation with threading (4 threads by default)
  $0 -P exp0_baseline

  # Parallel evaluation with 8 threads on GPU 2
  $0 -P -w 8 -g 2 exp0_baseline

  # Evaluate with specific config
  $0 -c configs/evaluation/eval_exp1.yaml exp1_remove_expletives
...
main() {
    ...
    if [[ "$config" == "$DEFAULT_CONFIG" ]] && [[ -n "$experiment" ]]; then
        ...
        # Case 1: experiment is like "exp0_baseline" -> look for "configs/evaluation/eval_exp0_baseline.yaml"
        if [[ -f "configs/evaluation/eval_${experiment}.yaml" ]]; then
            auto_config="configs/evaluation/eval_${experiment}.yaml"
        # Case 2: experiment is like "eval_exp0_baseline" -> look for "configs/evaluation/eval_exp0_baseline.yaml"
        elif [[ "$experiment" =~ ^eval_ ]] && [[ -f "configs/evaluation/${experiment}.yaml" ]]; then
            auto_config="configs/evaluation/${experiment}.yaml"
        fi
...
" 2>/dev/null || echo "models/$experiment/")
    ...
    # Run evaluation
    if run_evaluation "$temp_config" "$gpu_ids" "$checkpoint_path" "$verbose" "$force_rerun" "$parallel_workers" "$use_parallel"; then
