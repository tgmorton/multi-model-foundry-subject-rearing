...
const howToCode = [
  ...
  h2("Known bug, already pre-marked in two files"),
  calloutRich([
    ...
    new TextRun(", "),
    code("webeamus"),
    new TextRun(", and so on. "),
    new TextRun({ text: "We have already auto-marked every row containing one of these pseudo-words as ‘i’ (incorrect) with category_hit = ‘bug:contraction-glue’.", bold: true }),
    new TextRun(" Roughly 40–50% of the rows in Change 3 and Change 4 are already filled in. You can skim them to verify the bug is what we say it is, but you do not need to add a verdict to those rows. Focus your attention on the rows where the verdict column is "),
    new TextRun({ text: "blank", italics: true }),
    new TextRun(" — those are the cases the auto-marker didn't recognize, and they're where your judgment matters."),
  ], "E07B7B"),
...
const lemmatizeVerbsEn = [
  ...
  calloutRich([
    new TextRun({ text: "Known bug, already pre-marked: ", bold: true }),
    ...
    new TextRun(", "),
    code("wasn't"),
    new TextRun(" got glued to their replacements, producing pseudo-words like "),
    code("itbe"),
    new TextRun(", "),
    code("webe"),
    new TextRun(", "),
    code("ben't"),
    new TextRun(". Every row containing one of these is "),
    new TextRun({ text: "already marked", bold: true }),
    new TextRun(" with verdict = "),
    code("i"),
    new TextRun(" and category_hit = "),
    code("bug:contraction-glue"),
    new TextRun(". You only need to look at rows where the verdict column is blank — about 144 of the 250."),
  ], "E07B7B"),
...
const enrichEn = [
  ...
  calloutRich([
    new TextRun({ text: "Known bug, already pre-marked: ", bold: true }),
    new TextRun("same contraction-glue issue as Change 3, but it also combines with the new suffixes, producing pseudo-words like "),
    code("itbeat"),
    new TextRun(", "),
    code("webeamus"),
    new TextRun(", "),
    ...
    new TextRun(" and category_hit = "),
    code("bug:contraction-glue"),
    new TextRun(". You only need to look at rows where the verdict column is blank — about 130 of the 250."),
  ], "E07B7B"),
