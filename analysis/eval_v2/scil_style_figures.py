...
def multirun_curves(pairs: pd.DataFrame, ckpts: pd.DataFrame, arch: str,
    ...
    effective_batch_size is a swept HP, so the 10 runs' checkpoint_step
    grids don't align (h0/h1 reach 30k steps, h2-h4 only 15k for the same
    data). Averaging by raw step then pools a shifting, incomplete subset
    of runs per step → spurious late-training jitter. Instead we join each
    checkpoint to its tokens_seen (sidecar), bin tokens into log-spaced
    ...
    d = d.dropna(subset=["tokens"])
    d = d[d.tokens > 0]
    # Shared log-spaced token bins across all conditions for this arch.
    import numpy as np
    lo, hi = d.tokens.min(), d.tokens.max()
    edges = np.logspace(np.log10(lo), np.log10(hi), n_bins + 1)
    centers = np.sqrt(edges[:-1] * edges[1:])
    d["tbin"] = pd.cut(d.tokens, bins=edges, labels=False,
                       include_lowest=True)
    ...
    for ax, cat in zip(axes.flat, CATS):
        ...
        for cond in CONDS:
            ...
            ax.fill_between(x, stat["mean"].values - sd,
                            stat["mean"].values + sd,
