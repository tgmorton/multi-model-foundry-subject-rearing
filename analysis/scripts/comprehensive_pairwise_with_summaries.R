# Comprehensive Overt Preference Pairwise Comparisons - WITH SUMMARY TABLES
...
# Display B1 summary table
cat("\nB1: NUMBER CONTRASTS SUMMARY\n")
cat("============================\n")
if (length(b1_summary) > 0) {
  all_b1_summary <- bind_rows(b1_summary)
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(b1_summary)) {
      model_label <- b1_summary[[model_name]]$Model[1]
      model_data <- b1_summary[[model_name]]
...
for (model_name in unique(endstate_data$model)) {
  ...
  for (pair in person_pairs) {
    ...
    if (nrow(p1_data) > 0 && nrow(p2_data) > 0) {
      ...
      chi_test <- chisq.test(cont_table)
      comparison_name <- paste0(pair[1], " vs ", pair[2])
      
      # Store summary
      model_summary[[comparison_name]] <- tibble(
  ...
  # Store model summary
  if (length(model_summary) > 0) {
    b2_summary[[model_name]] <- bind_rows(model_summary) %>%
      mutate(model = model_name, Model = model_label, .before = 1)
  }
}

# Display B2 summary table
cat("\nB2: PERSON CONTRASTS SUMMARY\n")
cat("============================\n")
if (length(b2_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(b2_summary)) {
      model_label <- b2_summary[[model_name]]$Model[1]
      model_data <- b2_summary[[model_name]]
      
      cat(sprintf("\n%s:\n", model_label))
      for (i in 1:nrow(model_data)) {
        row <- model_data[i,]
        cat(sprintf("  %s: %.3f vs %.3f, diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                    str_pad(row$comparison, 20), row$prop1, row$prop2, row$diff,
                    row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
      }
    }
  }
...
for (model_name in unique(endstate_data$model)) {
  ...
  if (nrow(subj_data) > 0 && nrow(obj_data) > 0) {
    ...
    # Store detailed result
    b3_results[[model_name]] <- b3_summary[[model_name]]
  }
}

# Display B3 summary table
cat("\nB3: CONTROL CONTRASTS SUMMARY\n")
cat("=============================\n")
if (length(b3_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(b3_summary)) {
      row <- b3_summary[[model_name]]
      cat(sprintf("%s: Subject=%.3f, Object=%.3f, diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                  str_pad(row$Model, 25), row$subject_control, row$object_control, row$diff,
                  row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
    }
  }
}
...
for (model_name in unique(endstate_data$model)) {
  ...
  if (nrow(seems_data) > 0 && nrow(be_data) > 0) {
    ...
    # Store detailed result
    b4_results[[model_name]] <- b4_summary[[model_name]]
  }
}

# Display B4 summary table
cat("\nB4: EXPLETIVE CONTRASTS SUMMARY\n")
cat("===============================\n")
if (length(b4_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(b4_summary)) {
      row <- b4_summary[[model_name]]
      cat(sprintf("%s: Seems=%.3f, Be=%.3f, diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                  str_pad(row$Model, 25), row$expletive_seems, row$expletive_be, row$diff,
                  row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
    }
  }
}
...
for (model_name in unique(endstate_data$model)) {
  ...
  if (nrow(no_shift_data) > 0 && nrow(shift_data) > 0) {
    ...
    # Store detailed result
    b5_results[[model_name]] <- b5_summary[[model_name]]
  }
}

# Display B5 summary table
cat("\nB5: TOPIC SHIFT CONTRASTS SUMMARY\n")
cat("=================================\n")
if (length(b5_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(b5_summary)) {
      row <- b5_summary[[model_name]]
      cat(sprintf("%s: No_shift=%.3f, Shift=%.3f, diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                  str_pad(row$Model, 25), row$no_topic_shift, row$topic_shift, row$diff,
                  row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
    }
  }
}
...
for (model_name in unique(endstate_data$model)) {
  ...
  for (form_name in other_forms) {
    ...
    if (nrow(form_data) > 0) {
      ...
      # Store summary
      model_summary[[form_name]] <- tibble(
        form = form_name,
        form_prop = form_prop,
  ...
  # Store model summary
  if (length(model_summary) > 0) {
    c1_summary[[model_name]] <- bind_rows(model_summary) %>%
      mutate(model = model_name, Model = model_label, .before = 1) %>%
      arrange(desc(diff))  # Order by largest positive differences first
  }
}

# Display C1 summary table
cat("\nC1: FORMS VS DEFAULT SUMMARY\n")
cat("============================\n")
if (length(c1_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(c1_summary)) {
      model_label <- c1_summary[[model_name]]$Model[1]
      model_data <- c1_summary[[model_name]]
      
      cat(sprintf("\n%s:\n", model_label))
      for (i in 1:nrow(model_data)) {
        row <- model_data[i,]
        cat(sprintf("  %s: %.3f vs %.3f (default), diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                    str_pad(row$form, 18), row$form_prop, row$default_prop, row$diff,
                    row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
      }
    }
  }
...
for (model_name in unique(endstate_data$model)) {
  ...
  if (nrow(long_data) > 0 && nrow(emb_data) > 0) {
    ...
    # Store detailed result
    c2_results[[model_name]] <- c2_summary[[model_name]]
  }
}

# Display C2 summary table
cat("\nC2: COMPLEX EMBEDDING SUMMARY\n")
cat("=============================\n")
if (length(c2_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(c2_summary)) {
      row <- c2_summary[[model_name]]
      cat(sprintf("%s: Long=%.3f, Embedded=%.3f, diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                  str_pad(row$Model, 25), row$complex_long, row$complex_embedded, row$diff,
                  row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
    }
  }
}
...
for (model_name in unique(endstate_data$model)) {
  ...
  # Store model summary
  if (length(model_summary) > 0) {
    c3_summary[[model_name]] <- bind_rows(model_summary) %>%
      mutate(model = model_name, Model = model_label, .before = 1)
  }
}

# Display C3 summary table
cat("\nC3: NEGATION TYPE COMPARISONS SUMMARY\n")
cat("=====================================\n")
if (length(c3_summary) > 0) {
  for (model_name in unique(endstate_data$model)) {
    if (model_name %in% names(c3_summary)) {
      model_label <- c3_summary[[model_name]]$Model[1]
      model_data <- c3_summary[[model_name]]
      
      cat(sprintf("\n%s:\n", model_label))
      for (i in 1:nrow(model_data)) {
        row <- model_data[i,]
        cat(sprintf("  %s: %.3f vs %.3f, diff=%+.3f, χ²(1)=%.2f, p=%.4f %s\n",
                    str_pad(row$comparison, 30), row$prop1, row$prop2, row$diff,
                    row$chi_sq, row$p_value, ifelse(row$significant, "*", "")))
      }
    }
  }
