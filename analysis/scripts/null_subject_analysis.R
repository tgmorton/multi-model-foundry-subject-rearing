...
p_all_models_log_updated <- ggplot(all_models_summary, aes(x = checkpoint_log, y = mean_correct,
                                                   color = model_label, fill = model_label)) +
  geom_line(size = 0.4, alpha = 0.9) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.08, color = NA) +
  # Add t50 confidence intervals as rectangles (behind lines)
  geom_rect(data = model_comparison_crossover,
           aes(xmin = ci_lower_log, xmax = ci_upper_log, fill = model_label),
           ymin = 0, ymax = 1, alpha = 0.1, inherit.aes = FALSE) +
  # Add 50/50 acquisition points (lines on top of ribbons)
...
# Re-save the updated figure
save_paper_figure(p_all_models_log_updated, "analysis/paper_figures/wide/all_models_comparison_log", "wide")
