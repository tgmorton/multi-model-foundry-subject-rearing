"""
Impoverish Determiners Ablation Script

This script replaces all determiners with 'the' in the corpus.
This ablation tests how the model learns when determiner morphology is impoverished.

Procedure: ImpoverishDeterminers(text)
...
5.   if token.pos_ == 'DET':
6.     append 'the' + token.whitespace_ to modified_parts
7.   else:
8.     append token.text_with_ws to modified_parts
9.   end if
10. end for
11. result = join(modified_parts)
12. return result
"""
...
def impoverish_determiners_doc(doc, logger=None):
    """
    Replace all determiners with 'the' in a spaCy Doc object.
    ...
    for token in doc:
        if token.pos_ == "DET":
            ...
            num_replaced += 1
            if logger:
                logger.debug(f"Replaced determiner: '{token.text}' -> 'the'")
        else:
...
def ablate_doc(doc, logger=None):
    ...
    """
    return impoverish_determiners_doc(doc, logger)


# Removed local setup_logging function - now using unified logging from model_foundry.logging_utils


def validate_determiner_impoverishment(original_text, ablated_text, nlp):
    """
    ...
    Returns:
        bool: True if determiners were found and impoverished
    """
    ...
    if original_determiners:
        print(f"  Found {len(original_determiners)} determiners in original text")
        print(f"  Found {len(ablated_determiners)} determiners in ablated text")
        # Check if determiners were impoverished (all should be 'the')
        non_the_determiners = [d for d in ablated_determiners if d.lower() != 'the']
        if non_the_determiners:
            print(f"  Warning: Found {len(non_the_determiners)} non-'the' determiners in ablated text")
            return False
        else:
            print(f"  All determiners successfully impoverished to 'the'")
            return True
    else:
        print(f"  No determiners found in original text")
        return True


def main():
    parser = argparse.ArgumentParser(
        description="Impoverish determiners in a directory of .train files while maintaining dataset size."
    )
    ...
    parser.add_argument("--skip_validation", action="store_true", help="Skip determiner impoverishment validation to speed up processing.")
    ...
    for source_path in tqdm(source_files, desc="Processing Corpus Files"):
        ...
        with tqdm(
            ...
            for i in range(0, len(lines), args.chunk_size):
                ...
                docs = nlp.pipe(chunk)
                for doc in docs:
                    ablated_doc_text, num_replaced = ablate_doc(doc, logger)
                ...
                pbar.set_postfix(replaced=f'{stats["determiners_impoverished"]:,}')
        
        # Validate that determiners were actually impoverished (unless skipped)
        if not args.skip_validation:
            print(f"\n  Validating determiner impoverishment for {os.path.basename(source_path)}:")
            validation_success = validate_determiner_impoverishment(original_text, ablated_text, nlp)
            if not validation_success:
                print(f"  ⚠️  Warning: No determiners were impoverished from {os.path.basename(source_path)}")
        else:
        ...
        with tqdm(total=target_token_count, initial=current_token_count, desc="  Rebuilding to size", leave=False) as pbar_rebuild:
            while current_token_count < target_token_count:
                ...
                pbar_rebuild.set_postfix(replaced=f'{stats["determiners_impoverished"]:,}')
