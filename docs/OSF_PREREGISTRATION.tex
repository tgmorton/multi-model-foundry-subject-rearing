% NOTE (2026-04-20): this TeX is a hand-maintained draft and has drifted from
% docs/OSF_PREREGISTRATION.md, which is the canonical source. A regeneration
% pass to match the current .md is pending. The language pair has been updated
% from English+Italian to English+Spanish here for consistency with the .md,
% but specific Italian example sentences in the Target Grammatical Contexts
% section have NOT been re-translated — they remain as they were in the draft.
% Sync with the .md before submitting to OSF.
...
\textbf{Intervention:} [Baseline, Remove Expletives, Impoverish Verbal Morphology, Insert Verbal Morphology, Impoverish Case]

\textit{(Pronoun-distribution sweeps and determiner interventions from earlier plans are deferred; see .md canonical version.)}

\subsection{Spanish}

\textbf{Model:} [n-gram: 1-, 2-, 3-, 4-, 5-gram; GPT: small, medium, large; BERT: large; LSTM; and Mamba: 370m]

\textbf{Intervention Conditions:} [Baseline, Remove Expletives, Impoverish Verbal Morphology, Impoverish Case, Insert Pronouns]

\textit{(Determiner interventions from earlier plans are deferred; see .md canonical version.)}
...
In addition the model will be evaluated in the same way, on a suite of minimal pairs designed to target the model's preference on specific grammatical contexts relevant to the production of null and overt subjects and objects. This evaluation set is created parallel in English and Spanish, such that for each English pair there is a Spanish pair that is syntactically, if not semantically, equivalent, with differing or similar grammatical status. See the canonical .md version for current stimuli counts (1,488 rows / 744 paired items / 29 conditions / 11 categories including three Spanish-only categories). Each part of a pair consists of a context sentence and a target sentence. The evaluation stimuli were generated by Claude Opus 4.7 subagents following per-category translation rules (see \texttt{docs/eval\_stimuli/spanish.md}). Both English and Spanish sentences are verified by a fluent Peninsular-Spanish collaborator.
...
\begin{exe}
    ...
    \end{xlist}
    \ex \textbf{Spanish:} \% WARN: example sentences below are Italian from earlier draft; re-translate to Spanish before submitting.
    \begin{xlist}
