...
| D8 | Checkpoint paradigm for the wave | **RESOLVED 2026-08-22 (D8a memo + Thomas)** | Smoke verified bitwise CUDA determinism works (det1==det2 sha256, RTX-3090-pinned) at a measured **1.7× step-time tax**. Thomas: not worth it — **fast kernels everywhere, no deterministic subset**; if an intermediate state is ever truly needed, retrain from start. Sparse resumable anchors (~4/run: ep1, ep2, mid, final) + endpoint weights kept; everything else prunes after eval. The remorse-prevention burden moves to **preemptive capture** (D9). |
| D9 | Preemptive capture manifest | **PROPOSED 2026-08-22** | Bank per checkpoint, in the eval pod while weights are loaded anyway (training stays fast): (a) full matched eval suite incl. per-token logprobs; (b) held-out perplexity (test_10M slice); (c) **pronoun probe battery** — a frozen ~20K-instance sample of the scored corpus, stratified by decile × arm × genre × form, giving instance-level P(pronoun \| context) + inventory-slot distributions at every checkpoint (the dose-response trajectory at the resolution the study is about); (d) per-tensor weight-stat summaries (norms) before deletion; (e) training curves exported to S3 at run completion (not nightly); (f) last-layer reps for a small probe subset at the 4 anchors only. All S3-archived before any weight deletion; pruning gated on capture-complete markers. |
| D10 | Architecture set for the full v5 wave | **LOCKED 2026-09-18 (Thomas)** | **gpt2_large + bert_large + lstm only** — gpt2_small and gpt2_medium are dropped from the full-architecture runs (gpt2_small survives only as the already-running three-rater comparison cohort). Supersedes D5's cheap-arch-first staging for the main wave; brings the run requirement down from 5 archs to 3. |
| D11 | Composite ("comp") rater arm ended early | **2026-09-18 (Thomas)** | comp-arm training suspended mid-cohort to free namespace/utilization headroom ("odds are we're just going to run bert"). Completed comp runs (14+) stay banked; jobs suspended not deleted, resumable if the arm is revived. Shared rand/all100 controls keep running and still serve comp analyses. |
| D12 | gpt2_small cohort scope | **LOCKED 2026-09-21 (Thomas)** | The gpt2_small three-rater cohort **ends at h0** (112/112 active cells complete at h0; ~140 partial h1/h2 replicates banked as bonus). Enforced via `wave_control/w2v5.json` (`max_hp_rank: 0`) read by `wave2_agent.py` — running pods finish, new indices no-op. Rationale: the full wave re-trains on gpt2_large/bert_large/lstm (D10), so deeper gpt2_small seeds are not worth the GPUs. The `robbi` arm (D13) trains on gpt2_small at h0 only for the same reason. |
| D13 | Listener upper-bound arm | **2026-09-19 (advisor)** | Bidirectional roberta-large ±250 rater ("robbi") added to the v5 family as a tight listener-construal upper bound; 45 info cells, shares rand/all100 controls. Saturated (~53% < 0.1 nats) — interpret at decile granularity. |
...
- D2. Pool sufficiency for stacked cells — **COMPUTED 2026-08-23**.
  Non-expletive interventions are 1:1 token rewrites (zero word delta):
  their 80 cells compose STRICT with margins ≥1.30×. The
  remove_expletive_sentences × sweep cells compound line-removal with
  pronoun cost and CANNOT be word-matched from the existing pool in
  several genres (upper-bound margins at K=100: gutenberg 0.31×,
  bnc_spoken 0.52×, switchboard 0.74×, open_subtitles/simple_wiki
  ~1.01×). CRITICAL PRECEDENT: the existing single-intervention
  remove_expletive_sentences corpus ALREADY shipped short via
  --allow-short (gutenberg −3,127,412 words, pool exhausted; bnc −52K) —
  every model trained on that condition embodies this. INTERIM APPROACH
  (Thomas can revise): compose the 20 expletive cells with --allow-short
  (precedent-consistent, deficit grows with K and is recorded per cell
  in COMPOSE_MANIFESTs); the 80 other cells strict. Later options:
  (a) accept; (b) expand the gutenberg pool (easy source — more PG text
  through annotate→BERT-score→threshold; bnc/switchboard genuinely hard
  to expand); (c) global target-shrink to the expletive condition's
  totals (changes the matched-target design). Rebuilding 20 cells under
  (b)/(c) later costs ~1 h.
